"""实验四 红灯状态监测与事件日志分析 —— 完整答案脚本

运行前提：
1. CARLA 服务器已启动并运行在 localhost:2000；
2. 地图中存在交通灯（推荐 Town03 / Town05）；
3. 已安装 carla python API。
"""
...
def collect_events(world, ego_vehicle, traffic_lights, run_ticks=RUN_TICKS):
    ...
    for tick_index in range(run_ticks):
        ...
        if nearest_light is None or nearest_distance >= TRIGGER_DISTANCE_M:
            continue
