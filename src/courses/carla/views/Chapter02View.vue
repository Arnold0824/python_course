...
const expTemplateHref = "/courses/carla/ch02/exp01_sync_weather_camera_template.py";
const expAnswerHref = "/courses/carla/ch02/exp01_sync_weather_camera_answer.py";
    ...
    <main id="top" class="page is-slide-deck">
      ...
      >
        ...
        <div class="command-layout chapter-three-2plus1">
          ...
          <article class="command-card">
            <h3>先看最小现象</h3>
            <p>最先需要看到的是“多了一辆车”“相机开始出帧”“磁盘里多了图片”这类可观察结果，而不是大脚本里复杂的类结构。</p>
          </article>
          <article class="command-card">
            <h3>先会原理，再会扩展</h3>
      ...
      >
        ...
        <div class="command-layout chapter-three-2plus1">
          ...
          <article class="command-card">
            <h3>ego_vehicle</h3>
            <p><code>ego_vehicle</code> 只是变量名，不是特殊语法。它保存的是刚生成出来的那辆主车对象。</p>
          </article>
        </div>
      </section>
      ...
      >
        ...
        <p class="chapter-three-cue">
          下面 5 段代码建议在 notebook 中按顺序执行。先把环境初始化好，再去生成主车。这样后面的相机、队列、同步模式和资源清理都能直接复用。
        </p>
...
print("当前地图：", world.get_map().name)</code></pre>
          ...
          <article class="command-card chapter-three-cell-card">
            ...
            <h3>准备共享变量和清理函数</h3>
            <pre><code class="python">os.makedirs("output/ch02", exist_ok=True)
...
print("输出目录已准备：output/ch02")</code></pre>
          </article>
          <article class="command-card chapter-three-cell-card">
...
print("车辆位姿：", ego_vehicle.get_transform())</code></pre>
      ...
      >
        ...
        <div class="command-layout chapter-three-2plus1">
          ...
          <article class="command-card">
            ...
            <p>生成失败时返回 <code>None</code>，脚本不会自动中断。优点是方便先判断对象是否真的生成成功。</p>
          </article>
          <article class="command-card">
            <h3>这一章推荐用谁</h3>
            <p>实验中更推荐 <code>try_spawn_actor()</code>，因为它能直接暴露 <code>NoneType</code> 错误的来源。</p>
          </article>
        </div>
      </section>
      ...
      >
        ...
        <div class="concept-grid chapter-three-quad-grid">
          ...
          <article class="concept-card">
            <h3>基础检查</h3>
            <p>生成主车后先判断是否为 <code>None</code>。如果失败，就明确抛出“车辆生成失败”，再回到出生点或地图状态排查。</p>
          </article>
        </div>
      </section>
      ...
      >
        ...
        <div class="concept-grid chapter-three-quad-grid">
          ...
          <article class="concept-card">
            ...
            <p>这是一种简短写法，表示“每来一张图像，就把这张图像交给指定的处理逻辑”。</p>
          </article>
          <article class="concept-card">
      ...
      >
        ...
        <div class="chapter-three-code-stack">
          <article class="command-card chapter-three-cell-card">
            ...
            <h3>保存一张当前图像</h3>
            <pre><code class="python">os.makedirs("output/ch02", exist_ok=True)
filename = f"output/ch02/frame_{image.frame:06d}.png"
...
print("已保存：", filename)</code></pre>
      ...
      >
        ...
        <p class="chapter-three-cue">
          这一章的综合实验定为“同步天气控制与相机数据采集”。天气控制不是为了炫效果，而是为了看到：
        ...
        <div class="concept-grid chapter-three-quad-grid">
          ...
          <article class="concept-card">
            <h3>重要认知</h3>
            <p>天气变化影响的是视觉效果，不会自动省略同步模式和采集节奏问题。真正难点仍然是“切完天气以后，何时才保存图像”。</p>
          </article>
        </div>
      </section>
    ...
    path = f"output/ch02/{label}_{latest_image.frame:06d}.png"
    ...
    capture_weather_frame(label, weather)</code></pre>
      ...
      >
        ...
        <div class="command-layout chapter-three-2plus1">
          ...
          <article class="command-card">
            <h3>为什么这一步重要</h3>
            <p>这能让人第一次真正意识到：传感器数据不是“立刻同步出现”的，采集要考虑仿真节奏和数据延迟。</p>
          </article>
        </div>
      </section>
      ...
      >
        ...
        <div class="concept-grid chapter-three-quad-grid">
          ...
          <article class="concept-card">
            <h3>两者无关</h3>
            <p>即使不改 spectator，相机依然可以正常挂在车上并保存图片。这两件事非常容易被混成一件事。</p>
          </article>
          <article class="concept-card">
            <h3>本章建议</h3>
...
for step in range(20):
    ...
    if step % 5 == 0:
        path = f"output/ch02/autopilot_{image.frame:06d}.png"
...
print("第二章主线代码已清理完成")</code></pre>
      ...
      >
        ...
        <div class="command-layout chapter-three-2plus1">
          <article class="command-card">
            <h3>步骤 1 和 2</h3>
            <p>先保存原始设置，再开启同步模式和固定步长；然后生成主车。</p>
          </article>
          <article class="command-card">
            <h3>步骤 3 和 4</h3>
...
def main():
    ...
    try:
        ...
        # ego_vehicle = ___________________________
        # if ego_vehicle is None:
      ...
      >
        ...
        <div class="command-layout chapter-three-link-grid chapter-three-link-grid--balanced">
          <article class="command-card">
            ...
            <a class="chapter-three-link" :href="chapterAllCodeHref" download="carla_ch02_all_examples.py">
